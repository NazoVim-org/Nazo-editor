...
async fn vim_redo_restores_after_undo() {
    ...
    // 'u' undo
    ed.handle_key_for_test(KeyCode::Char('u'), KeyModifiers::NONE).await;
    let (buf, _, _, _) = ed.snapshot_for_test();
    assert_eq!(buf, "hello\nworld\n", "after undo");

    // Ctrl-R redo
    ed.handle_key_for_test(KeyCode::Char('r'), KeyModifiers::CONTROL).await;
    let (buf, _, _, _) = ed.snapshot_for_test();
    assert_eq!(buf, "world\n", "after redo");
}
...
async fn vim_page_down_moves_cursor_down() {
    ...
    ed.handle_key_for_test(KeyCode::Char('f'), KeyModifiers::CONTROL).await;
...
async fn vim_page_up_after_page_down_returns() {
    let mut ed = test_vim_editor("line1\nline2\nline3\nline4\nline5");
    // Go to bottom
    ed.handle_key_for_test(KeyCode::Char('G'), KeyModifiers::NONE).await;
    let (_, line, _, _) = ed.snapshot_for_test();
    assert_eq!(line, 5, "G goes to last line (5)");

    // Ctrl-B (page up) should move cursor up
    ed.handle_key_for_test(KeyCode::Char('b'), KeyModifiers::CONTROL).await;
    let (_, line, _, _) = ed.snapshot_for_test();
...
async fn vim_visual_yank() {
    ...
    ed.handle_key_for_test(KeyCode::Char('v'), KeyModifiers::NONE).await;
    // move right twice to select "abc"
    ed.handle_key_for_test(KeyCode::Char('l'), KeyModifiers::NONE).await;
    ed.handle_key_for_test(KeyCode::Char('l'), KeyModifiers::NONE).await;
    // yank
    ed.handle_key_for_test(KeyCode::Char('y'), KeyModifiers::NONE).await;
    // buffer unchanged
...
async fn vim_insert_undo_removes_inserted_text() {
    ...
    ed.handle_key_for_test(KeyCode::Char('i'), KeyModifiers::NONE).await;
    for ch in "xyz".chars() {
        ed.handle_key_for_test(KeyCode::Char(ch), KeyModifiers::NONE).await;
    }
...
async fn vim_insert_enter_undo() {
    ...
    ed.handle_key_for_test(KeyCode::Char('l'), KeyModifiers::NONE).await;
    ed.handle_key_for_test(KeyCode::Char('i'), KeyModifiers::NONE).await;
    ed.handle_key_for_test(KeyCode::Enter, KeyModifiers::NONE).await;
    ed.handle_key_for_test(KeyCode::Esc, KeyModifiers::NONE).await;
    let (buf, _, _, _) = ed.snapshot_for_test();
